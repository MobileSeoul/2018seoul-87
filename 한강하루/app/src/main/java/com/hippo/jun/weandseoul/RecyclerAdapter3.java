...
public class RecyclerAdapter3 extends RecyclerView.Adapter<RecyclerView.ViewHolder>  {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ViewHolder(View view){
            ...
            cardTxt = view.findViewById(R.id.card_txt1);
            cardView = view.findViewById(R.id.cardView);
        }
    }
    //생성자
    public RecyclerAdapter3(Context context, ArrayList<RecyclerItem> recyclerItems) {
        mContext = context;
        this.recyclerItems = recyclerItems;
    }
    public RecyclerAdapter3(Context context, ArrayList<RecyclerItem> recyclerItems, Intent intent) {
        mContext = context;
        this.recyclerItems = recyclerItems;
        this.mIntent = intent;
    }
...
//        Glide.with(mContext).load(recyclerItems.get(position).url).fitCenter().into(viewHolder.cardImg);
        ...
        Glide.with(mContext).load(recyclerItems.get(position).url).into(viewHolder.cardImg);
//        viewHolder.tel.setText(recyclerItems.get(position).tel);
//        viewHolder.info.setText(recyclerItems.get(position).info);
//        viewHolder.xCode.setText(recyclerItems.get(position).xCode);
//        viewHolder.yCode.setText(recyclerItems.get(position).yCode);

        if(mIntent != null){
            viewHolder.cardView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
//                    mIntent.putExtra("name", viewHolder.cardTxt.getText());
//                    mIntent.putExtra("tel", viewHolder.tel.getText());
//                    mIntent.putExtra("info", viewHolder.info.getText());
//                    mIntent.putExtra("xCode", viewHolder.xCode.getText());
//                    mIntent.putExtra("yCode", viewHolder.yCode.getText());
